...
**Point-head supervision (pi3x / GT / dual).** The `PointHead` (`gr00t/model/backbone/point_head.py`) can be supervised from two interchangeable on-disk caches that share the same npz schema (`xy`/`log_z`/`conf` per cam per episode, full-res 224×224):
...
**PointHead output resolution (16 vs 224).** `--point-head-output-resolution {16,224}` (default **16**) selects the head topology, mirroring openpi's `aux_point_head.output_resolution`. **16:** per-patch (16×16) prediction via a `Linear` head; the dataset avg-pools the cache frame to the 16×16 patch grid. **224:** full-resolution prediction via a Pi3X-style `_ConvHeadUpsampler` (16→32→64→128 ConvTranspose2d, bilinear→224, zero-init xy/z conv heads — ported verbatim from openpi); the dataset keeps the **un-pooled** 224×224 targets. The flag threads `gr00t_finetune.py → from_pretrained → backbone_cfg["pi3x_output_resolution"] → EagleBackbone → build_geometry_modules → PointHead`, and in parallel sets `CameraAwareLeRobotDataset(point_target_resolution=)` so the target resolution matches the head (mismatch raises). openpi's configs use 224; the 16-only port was the historical default here. Step-0 head output stays exactly zero in both modes (`reset_to_zero` resets conv layers and re-zeros the final output conv), preserving the bf16-safe baseline.

**npy-mmap point cache (loading-perf fix).** The on-disk caches are **compressed** `.npz` (one per episode, `(T,224,224,·)`). `np.load(mmap_mode="r")` is **silently ignored for compressed npz**, so reading one frame decompresses the *whole episode* (~460–630 ms/cam) — this CPU-starves the GPUs and gets jobs killed for low GPU utilisation (the 16×16 GPU step is too cheap to hide it; openpi's 224 step hides it). `CameraAwareLeRobotDataset._load_cam_frame` therefore prefers an **uncompressed per-array `.npy`** layout — `{cam}/episode_{NNNNNN}/{xy,log_z,conf}.npy` — which truly mmaps one frame (~34 ms/sample, ~13× faster), falling back to `.npz` when absent. Re-pack a compressed cache with `scripts/sbatch_lg/convert_point_cache_to_npy.py` (numerically identical; preserves float16). Existing `.npy` re-packs under `/scratch/lg154/cache/point_targets_npy/`: `robocasa24_all24_human_camaware` (subdirs `base`/`wrist`), `libero_gt_cam_v2_aligned` and `libero_pi3x_cam_v2` (subdirs `agent`/`wrist`). The `*_res224.sbatch` recipes point at these; the 16×16 recipes still read the original compressed `.npz` caches.
